...
- 나머지 처리는 Cloud Functions `onUserDeleted` 트리거가 Admin 권한으로 서버에서 처리
  - 본인이 쓴 리뷰/게시글은 댓글·좋아요·스크랩 서브컬렉션까지 통째로 삭제 (`recursiveDelete`)
  - 다른 사람 글에 남긴 댓글/답글도 삭제 — 댓글에 다른 사람이 남긴 답글이 있으면 스레드 보존을 위해 내용만 비우는 소프트 삭제, 없으면 완전 삭제. 답글은 항상 완전 삭제. 두 경우 다 부모 글의 `commentCount`는 갱신됨
  - 잉크북/팔로우/알림/스토리지 삭제, 팔로우·좋아요 카운트 정합성 유지
  - (예전엔 콘텐츠를 지우지 않고 작성자 닉네임만 "알 수 없음"으로 바꿔 남겼었는데, 좋아요 시 작성자에게 EXP를 지급하는 트랜잭션이 이미 삭제된 `users/{authorId}` 문서를 건드리려다 실패하는 문제가 있어 완전 삭제로 전환함)
