...
- **인기글 섹션** (전체 탭일 때만 표시)
  ...
  - 각 카드: 제목 + 본문 미리보기 + 작성자 CircleAvatar(radius 10) + 닉네임(네이비) + 작성 시간 (카테고리 뱃지는 없음)
...
- **전체 게시글 리스트 (`PostCard`)**
  ...
  - 카드 탭 → `/community/:postId` (프사·닉네임 포함 카드 어디를 눌러도 동일 — 목록에서는 실수로 프로필 화면으로 이동하지 않도록 프사/닉네임 개별 탭 네비게이션 없음)
- 무한 스크롤 (pageSize 20)
- 게시글 사이 Divider 구분선
