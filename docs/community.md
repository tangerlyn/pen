...
- **전체 게시글 리스트 (`PostCard`)**
  ...
  - 좋아요/댓글 수는 표시만 되고 탭 불가(스크롤 중 오탭 방지) — 좋아요는 게시글 상세에서만 토글 가능
...
- 무한 스크롤 (pageSize 20)
- 게시글 사이 Divider 구분선
- **하단 우측 FAB** ("글 쓰기" 텍스트 버튼, 아이콘 없음) → `/community/write`
...
- 본문 — 블로그 에디터 (텍스트 블록 + 이미지 블록 혼합 입력, `BlogBodyEditor`, 리뷰 작성과 동일 컴포넌트)
  ...
  - 본문 텍스트 블록에 포커스가 있는 동안에만(제목칸 포커스 시엔 안 보임) 키보드 바로 위에 네이버 블로그 스타일 고정 툴바(`BlogEditorToolbar`)가 나타남 — 카메라 / 사진 아이콘 (별도 링크 삽입 버튼은 없음, 아래 참고)
  ...
  - **링크는 별도 버튼 없이 자동 감지** — 본문에 `http(s)://`로 시작하는 URL을 직접 입력/붙여넣기하면, 상세 화면에서 표시될 때 `LinkifiedText`(`lib/shared/widgets/linkified_text.dart`)가 자동으로 밑줄 표시된 탭 가능한 링크로 렌더링함. 탭하면 외부 브라우저 앱으로 나가지 않고 인앱 브라우저(`url_launcher`의 `LaunchMode.inAppBrowserView`)로 열림
