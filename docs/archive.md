...
- **AppBar**
  ...
  - 우측 + 아이콘 → 제품 직접 등록 바텀 시트
- **잉크 탭**
  - 브랜드 드롭다운 필터
  - 색상 계열 멀티 선택 칩 (레드·오렌지·옐로우·그린·시안·블루·퍼플·핑크·무채색·기타)
  - 종류 멀티 선택 칩 (일반·펄·쉰)
  - 잉크 목록 리스트 (`InkListTile`) — 탭 → `/archive/ink/:productId`
- **만년필 탭**
  - 브랜드 / 펜촉 소재 / 잉크 주입 방식 / 펜촉 굵기 드롭다운 필터
  - 만년필 목록 리스트 (`PenListTile`) — 탭 → `/archive/pen/:productId`
- 필터 적용 시 상단에 활성 필터 칩 표시, X 버튼으로 개별 초기화
- 스크롤 맨 위로 버튼
