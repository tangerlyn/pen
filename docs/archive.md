...
- **잉크 탭**
  ...
  - 색상 계열 / 특수 속성 필터 — 바텀시트가 아니라, 칩을 누르면 그 칩이 그대로 헤더가 되어 바로 아래로 옵션 목록이 펼쳐지는 드롭다운(토스 스타일). 칩 위치/크기에 `CompositedTransformTarget`·`Follower`로 앵커링되고, 선택 개수에 맞춰 헤더 폭이 실시간으로 늘어남(`IntrinsicWidth`, 화면폭의 75%까지). 바깥 영역은 어둡게 딤 처리, 바깥 탭이나 하단 "완료" 버튼으로 닫힘
    - 색상 계열: 빨강·주황·노랑·초록·파랑·보라·검정 7개 (`InkModel.autoColorFamily`가 hexColor의 HSV로 자동 분류)
    - 특수 속성: 일반·펄·테
    - 둘 다 다중 선택 가능, 목록 맨 위 "전체" 항목으로 선택 해제. 고르는 동안은 로컬에만 임시로 담아두고, 닫힐 때만 실제 필터에 반영(고를 때마다 목록이 바로 바뀌지 않도록)
  - 정렬 칩 (기본순·인기순) — "기본순"은 이름 가나다순(한글 우선) + 영어/숫자로 시작하는 이름은 뒤로 밀려 알파벳/숫자 순 (별점순은 별점 시스템 비활성화로 목록에서 숨김 — `ArchiveSortOption.rating`은 enum에 남아있으나 UI에서 필터링됨)
  - 리스트 하단 "새 잉크 직접 등록하기" 버튼 → 제품 등록 바텀 시트
- **만년필 탭**
  - `PenListTile` 목록 (탭 → `/archive/pen/:productId`)
  - 정렬 칩만 제공 (별도 필터 칩 없음) — "기본순"은 잉크와 동일하게 이름 가나다순/알파벳순
