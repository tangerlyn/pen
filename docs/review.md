...
- **그리드 뷰 (`ReviewFeedCard`)** — 2열, 카드 간 간격 1px로 빈틈없이 붙어서 표시(아카이브 상세 리뷰 그리드와 동일한 방식)
  - 사진 좌측 상단: 별점 뱃지 / 우측 상단: 사진 여러 장이면 장수 뱃지
  - 사진 하단: 태그된 잉크·만년필 중 첫 번째 이름을 흰 글씨(그림자 처리)로 사진 위에 직접 오버레이 표시 (ex. "블랙 45 +4" — 여러 개면 나머지 개수를 "+N"으로 표시), 태그 없으면 표시 안 함
  - 사진 아래 정보 영역: 작성자 프사·닉네임 + 제목만 표시 (좋아요·댓글·작성시간은 그리드에서 생략)
