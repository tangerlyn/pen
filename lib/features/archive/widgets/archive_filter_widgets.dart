...
class _FilterRowState extends ConsumerState<_FilterRow>
  ...
  // 목록이 바로바로 바뀌지 않도록 함.
  final _draft = ArchiveFilterDraftController();
  List<String> get _pendingColorFamilies => _draft.colorFamilies;
  List<String> get _pendingInkTypes => _draft.inkTypes;
  List<String> get _pendingBrands => _draft.brands;
  List<String> get _pendingFillTypes => _draft.fillTypes;
  ...
  void _togglePending(_ExpandedFilter filter, String value) {
    final field = switch (filter) {
      _ExpandedFilter.color => ArchiveFilterDraftField.color,
      _ExpandedFilter.type => ArchiveFilterDraftField.inkType,
      _ExpandedFilter.brand => ArchiveFilterDraftField.brand,
      _ExpandedFilter.fillType => ArchiveFilterDraftField.fillType,
      _ExpandedFilter.sort => null,
    };
    if (field == null) return;
    _draft.toggle(field, value);
  ...
  void _clearPending(_ExpandedFilter filter) {
    ...
    if (field != null) _draft.clear(field);
    _overlayEntry?.markNeedsBuild();
  }
  ...
  void _toggleDropdown(
    ...
    _draft.reset(
      colorFamilies: inkCurrent.colorFamilies,
      inkTypes: inkCurrent.inkTypes,
      brands: widget.tabIndex == 0 ? inkCurrent.brands : penCurrent.brands,
      fillTypes: penCurrent.fillTypes,
    );
